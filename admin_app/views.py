...
from django.db.models.functions import ExtractWeek, ExtractYear, ExtractMonth, ExtractDay, Cast
...
def admin_order_management(request):
    # Prefetch items
    items_qs = OrderItem.objects.select_related('product', 'variant').prefetch_related('orderreturn')
...
def brand_management(request):
    ...
    if search_query:
        brands = Brand.objects.filter(Q(name__icontains=search_query) | Q(description__icontains=search_query)).order_by('-id')
    else:
...
def manage_offers(request):
    ...
    for offer in page_obj:
        ...
        if offer.offer_type == 'product':
            ...
            products_ids_str = ", ".join(str(p.id) for p in offer.products.all())
        elif offer.offer_type == 'category':
            applied_items = [category.name for category in offer.categories.all()]
            categories_ids_str = ", ".join(str(c.id) for c in offer.categories.all())
        ...
        offers_with_details.append({
            ...
            'products_ids_str': products_ids_str,
            'categories_ids_str': categories_ids_str
        })
    ...
    return render(request, 'offer_management.html', {
        'offers_with_details': offers_with_details,
        'products': Product.objects.all(),
        'categories': Category.objects.all(),
        'page_obj': page_obj,
        'search_query': search_query,
